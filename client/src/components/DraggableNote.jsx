...
function DraggableNote({
  ...
  return (
    <Draggable
      ...
      bounds="parent"
      position={note.position}
      onStart={() => bringToFront(note._id)} // ✅ bring forward on drag
      onStop={(e, data) => {
        handleUpdate(note._id, {
          position: { x: data.x, y: data.y },
        });
    ...
    >
      <div
        ref={nodeRef}
        onMouseDown={() => bringToFront(note._id)} // ✅ bring forward on click
        style={{
          position: "absolute",
          zIndex: zIndex, // ✅ dynamic stacking
          transform: `rotate(${index % 2 === 0 ? "-4deg" : "3deg"})`,
          fontFamily: "'Patrick Hand', cursive",
        }}
        className={`absolute p-6 w-72 shadow-2xl cursor-move ${
        ...
        {editingId === note._id ? (
          ...
          <>
            <h2
              className="text-xl italic mb-3"
              onClick={() => {
                bringToFront(note._id); // ✅ bring forward when expanding
                setExpandedId(
                  expandedId === note._id ? null : note._id
                );
              }}
            >
              {note.title}
